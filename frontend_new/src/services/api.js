...
export const mentorApi = {
  ...
  getProfile: (userId) => apiRequest(`/api/mentor/profile/${userId}`),
  ...
  getMyMentors: () => apiRequest("/api/mentor/my-mentors"),

  getMyPendingRequests: () => apiRequest("/api/mentor/my-requests"),
...
export const trophyApi = {
  ...
  getTrophy: (trophyId) => apiRequest(`/api/trophy/${trophyId}`),

  getTrophySvg: (trophyId) => `${API_BASE}/api/trophy/${trophyId}/svg`,
  ...
  getTrophyTypes: () => apiRequest("/api/trophy/types"),
};
...
export const ragApi = {
  ...
  getSuggestedQuestions: (repoName = null) =>
    apiRequest(
      `/api/rag/suggestions${repoName ? `?repo_name=${repoName}` : ""}`,
    ),
};
...
export const chatApi = {
  ...
  getSession: (sessionId) => apiRequest(`/api/mentorship/session/${sessionId}`),
  ...
  sendMessage: (sessionId, senderId, senderUsername, data) =>
    apiRequest(
      `/api/mentorship/session/${sessionId}/message?sender_id=${senderId}&sender_username=${senderUsername}`,
      {
        method: "POST",
        body: JSON.stringify(data),
      },
...
export const communityApi = {
  // Nudges
  getStuckContributors: () => apiRequest("/api/community/nudges/stuck"),
  ...
  getResourcesForTopic: (topic, repoName = null) =>
    apiRequest(
      `/api/community/resources/topic/${topic}${repoName ? `?repo_name=${repoName}` : ""}`,
    ),
  ...
  markResourceHelpful: (resourceId, userId) =>
    apiRequest(
      `/api/community/resources/${resourceId}/helpful?user_id=${userId}`,
      { method: "POST" },
    ),
...
export const gamificationApi = {
  ...
  getLeaderboard: (days = 30, limit = 10) =>
    apiRequest(
      `/api/spark/gamification/leaderboard?days=${days}&limit=${limit}`,
    ),
  ...
  // Badges
  getAllBadges: () => apiRequest("/api/spark/badges/all"),

  getUserBadges: (username) => apiRequest(`/api/spark/badges/user/${username}`),
...
export const analyticsApi = {
  ...
  getTopContributors: (days = 90, limit = 10) =>
    apiRequest(
      `/api/spark/analytics/invisible-labor/top/${limit}?days=${days}`,
    ),

  getUserMetrics: (username, days = 90) =>
    apiRequest(
      `/api/spark/analytics/invisible-labor/user/${username}?days=${days}`,
    ),

  getRepoSummary: (owner, repo, days = 90) =>
    apiRequest(
      `/api/spark/analytics/invisible-labor/repo/${owner}/${repo}?days=${days}`,
    ),
};
...
export const claimApi = {
  ...
  getMyClaimedIssues: () => apiRequest("/api/contributor/my-claimed-issues"),

  updateClaimActivity: (issueId) =>
    apiRequest(`/api/contributor/claim-activity/${issueId}`, {
      method: "POST",
    }),
};
...
export const messagingApi = {
  ...
  getConversations: () => apiRequest("/api/messaging/conversations"),

  markRead: (otherUserId) =>
    apiRequest(`/api/messaging/mark-read/${otherUserId}`, { method: "POST" }),

  // Mentorship Management
  getMentorshipRequests: () => apiRequest("/api/messaging/mentorship/requests"),
  ...
  getMentees: () => apiRequest("/api/messaging/mentees"),
...
export const profileApi = {
  getProfile: (username) => apiRequest(`/api/profile/${username}`),
...
export const repositoryApi = {
  getAllRepositories: () => apiRequest("/api/repositories"),

  getMaintainerRepos: () => apiRequest("/api/repositories/maintainer"),

  getContributorRepos: () => apiRequest("/api/repositories/contributor"),
